...
const AddMoneyOrWithdraw = (
    ...
    const { pots, addToPot, withdrawFromPot } = usePotsStore()
    ...
    const handleSubmit = (e) => {
        ...
        if (actionType === "add") {
            addToPot(currentPot.id, numericAmount)
          toast.success(`R${numericAmount} added successfully`)
        } else if (actionType === "withdraw") {
            withdrawFromPot(currentPot.id, numericAmount);
    ...
    const percentage = Math.floor((Number(amount) / currentPot.amount) * 100)
    ...
    <div className='potsForm fixed flex items-center transition-all duration-500 justify-center top-0 left-0 w-full h-screen'>
        <div className='bg-white max-w-md w-full p-5 rounded-md'>
            <div className='flex justify-between items-center mb-4'>
                <p className='font-semibold text-md text-2xl'>{actionType === "add" ? "Add to " : "Withdraw from "}'{currentPot.name}'</p>
            ...
            <div className='h-2 w-full bg-gray-200 rounded-3xl mt-2'>
                <div className='h-full rounded-3xl bg-purple-400' style={{ backgroundColor: currentPot.theme, width: `${percentage}%` }}></div>
            </div>
            ...
            <form onSubmit={handleSubmit} action="" className='flex flex-col'>
                ...
                />
                <button type='submit' className='bg-gray-950 mt-5 cursor-pointer hover:bg-gray-700 py-2.5 rounded-md text-white w-full'>Confirm {actionType === "add" ? "addition" : "withdrawal"}</button>
            </form>
        </div> 
    </div>
  )

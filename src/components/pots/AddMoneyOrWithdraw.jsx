...
const AddMoneyOrWithdraw = ({seShowAddMoneyOrWithdrawModal, actionType, pot}) => {
    ...
    <div className='potsForm fixed flex items-center transition-all duration-500 justify-center top-0 left-0 w-full h-screen'>
        <div className='bg-white max-w-md w-full p-5 rounded-md'>
            <div className='flex justify-between items-center mb-4'>
                <p className='font-semibold text-md text-2xl'>{actionType === "add" ? "Add to " : "Widthdraw from "}{pot.potName}</p>
                <button onClick={() => seShowAddMoneyOrWithdrawModal(false)}><img src={close} alt="" className='size-5 cursor-pointer'/></button>
            </div>
            ...
            <div className='flex items-center justify-between mt-4'>
                ...
                <span className='text-lg font-semibold'>R{actionType === "add" ? pot.amount + amount : pot.amount - amount }</span>
            </div>
